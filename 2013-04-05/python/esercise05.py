...
finestra1 = CUBOID([0.84,0.25,0.84])
fin_trasl = T([1,2,3])([7.16,6.5,1.66])(finestra1)
finestra_nera = COLOR(BLACK)(fin_trasl)

finestra2 = CUBOID([1.23*2,0.25,1.23])
fin2_trasl = T([1,2,3])([5.12,6.5,2.5+0.39+1.27])(finestra2)
finestra2_nera = COLOR(BLACK)(fin2_trasl)
...
scale2D = MKPOL([[[0,0],[0,2.65],[2.66,2.5/2],[2.66,2.65]],[[1,2,3,4]],None])
scale3D = PROD([step2D,Q(9)])
scale = MAP([S1,S3,S2])(scale3D))
scale3Dscambio = MAP([S1,S3,S2])(PROD([step2D,Q(9)]))
rampa = STRUCT(NN(10)([scale3Dscambio,T([1,3])([2.66,2.5/2])]))


buildings = STRUCT([pillars0,pillars1,pillars2, pillars3, floors, west, east, south, north,finestra_nera,finestra2_nera])
