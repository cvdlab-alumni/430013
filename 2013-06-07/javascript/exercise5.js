...
//parte lago grande
DRAW(genera_alberi_random(10,-10,40))
DRAW(genera_alberi_random(15,-10,40))
DRAW(genera_alberi_random(18,-15,40))
DRAW(genera_alberi_random(18,-10,40))
DRAW(genera_alberi_random(15,-10,40))

//parte lago piccolo
DRAW(genera_alberi_random(-10,10,50))
DRAW(genera_alberi_random(-10,20,50))
...
///EXERCISE 4-5 insieme modificato con strade
...
function posiz_case_e_strade(num_case_max,righe,area_x,area_y){
	var array_misto = new Array();
	...
	for(var j=0; j<righe;j++){	
		...
		for(var i=0; i<numero_c; i++){
			...
			array_misto.push(casa_xy)

			if(j>0){
				var strada = T([0,1])([-(area_x/righe)/2,spazio_y*j-spazio_y/2])(CUBOID([area_x+(area_x/righe),1*(area_y/10)/3,0.01]))
				array_misto.push(strada)}
		}
	}
	// var strada_diagonale = 

	var struct_case_e_strade = STRUCT(array_misto)
	
	return struct_case_e_strade;
}

var cuboid1 = COLOR(rgb(marrone))(T([0,1])([-38,-32])(CUBOID([35,30,0.7])))
var insediamento1 = T([0,1,2])([-35.5,-30,0.75])(posiz_case_e_strade(9,6,30,30))

var cuboid2 = COLOR(rgb(marrone))(T([0,1])([-10,24])(CUBOID([8,9,0.7])))
var insediamento2 = T([0,1,2])([-9,25,0.75])(posiz_case_e_strade(3,5,5,7))
